import { Url } from "../../support/types/urls";
...
export const productIdUrl = (index: number) =>
  `https://demoblaze.com/prod.html?idp_=${index}`;

export const apiRequest = (method: string, url: Url, body?: object) => {
  return body ? cy.request({ method, url, body }) : cy.request({ method, url });
};
